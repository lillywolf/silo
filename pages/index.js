    ...
    <div className={styles.container}>
      ...
      <main className={ `${ styles.main } m-auto relative` }>
        <div className='relative'>
          ...
          <div className='text-center pb-8 pt-4 m-auto sm:w-[620px] lg:w-[800px]'>
        ...
        <div className='mx-auto w-10/12 md:w-7/12'></div>
          <div className="flex flex-wrap sm:w-10/12 lg:w-7/12 mx-auto">
              { events.data.filter((event) => event.status !== 'cancelled').map((event) => <EventComponent key={event.id} event={event} />) }
          </div>
        </div>
      </main>
    </div>
